...
export const СhooseAnalistVariant = ({ obj }: IProps): JSX.Element => {
    console.log("СhooseBackendVariant get obj = ", obj)
    const objComponents = obj.split(",", 3);
    const objStr = obj;
    console.log("СhooseBackendVariant made new objStr = : " + objStr);

    const renderContent = (): JSX.Element => {
        switch (objComponents[0]) {
            case "1":
                return  <AnalistForm1 obj={objStr}/>;
            case "2":
                return  <AnalistForm2 obj={objStr}/>;
            case "3":
                return  <AnalistForm3 />;
            case "4":
                return  <AnalistForm4 />;
    ...
    for (const v of obj) {
        if(v.checked){
            ...
            if(back_tag === 19) {
                ...
                xhr_back.open("POST", 'http://localhost:8081/taskHardnessCalc/v1/taskBack')
