...
import {sqldev_job_string} from "./SqlTable";
...
export const SqlForm2 = observer((): JSX.Element => {
  ...
  const onChange = (e: RadioChangeEvent) => {
    ...
    SqlStore.setJobFactor(Number(e.target.value));
  };

  // const navigate = useNavigate();

  const handleClick = () : void => {
    SqlStore.setFactor(3);
        ...
        {/*<Form.Item label="Факторы влияющие на трудоёмкость задач для аналитика">*/}
          ...
          <Radio.Group onChange={onChange} value={value}>
            <Space direction="vertical">
              <Radio value={0}>1. {sqldev_job_string[0]} [0 storypoints]</Radio>
              <Radio value={1}>2. {sqldev_job_string[1]} [1 storypoints]</Radio>
              <Radio value={2}>3. {sqldev_job_string[2]} [2 storypoints]</Radio>
              <Radio value={3}>4. {sqldev_job_string[3]} [3 storypoints]</Radio>
              <Radio value={5}>5. {sqldev_job_string[5]} [5 storypoints]</Radio>
              <Radio value={8}>6. {sqldev_job_string[5]} [8 storypoints]</Radio>
            </Space>
          </Radio.Group>
        </Form.Item>
